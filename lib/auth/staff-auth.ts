...
export async function completeTotp(raw:string,code:string,ctx:Ctx={}){
 ...
 const v=verifyTotp(r.secret,code,r.last);if(!v.valid){const [s]=await db.select({n:staffSessions.totpFailures}).from(staffSessions).where(eq(staffSessions.id,r.sid)).limit(1);const n=(s?.n??0)+1;await db.update(staffSessions).set({totpFailures:n,revokedAt:n>=MAX?new Date():null}).where(eq(staffSessions.id,r.sid));return{status:"invalid"} as const;}
 const spent=await db.update(users).set({totpLastCounter:v.counter!,failedLoginCount:0,lockedUntil:null,lastLoginAt:new Date()}).where(and(eq(users.id,r.uid),r.last==null?isNull(users.totpLastCounter):eq(users.totpLastCounter,r.last))).returning({id:users.id});if(!spent.length)return{status:"invalid"} as const;
 const fresh=newToken(),now=Date.now();await db.insert(staffSessions).values({userId:r.uid,tokenHash:hash(fresh),totpVerifiedAt:new Date(),expiresAt:new Date(now+IDLE),absoluteExpiresAt:new Date(now+ABSOLUTE),ip:ctx.ip??null,userAgent:ctx.userAgent??null});await db.update(staffSessions).set({revokedAt:new Date()}).where(eq(staffSessions.id,r.sid));
