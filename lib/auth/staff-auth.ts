...
export async function completeTotp(raw:string,code:string,ctx:Ctx={}){
 ...
 const v=verifyTotp(r.secret,code,r.last);if(!v.valid){await db.update(staffSessions).set({revokedAt:new Date()}).where(eq(staffSessions.id,r.sid));return{status:"invalid"} as const;}
